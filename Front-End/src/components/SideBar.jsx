    ...
    const handleLinkClick = () => {
        if (window.innerWidth < 992) {
            ...
            if (menu && menu.classList.contains('show')) {
                ...
                setTimeout(() => menu.style.visibility = '', 300);
            }
        }
    };
        ...
        <div 
            ...
            <div className="offcanvas-body p-3 d-flex flex-column h-100 overflow-y-auto overflow-x-hidden" id="scroll">
                ...
                <ul className="nav flex-column gap-1 flex-grow-1">
                    {menuItems.map((item) => (
                        <li className="nav-item" key={item.path}>
                            <Link  to={item.path}  onClick={handleLinkClick}  className={`nav-link rounded-3 px-3 py-2 d-flex align-items-center mb-1 transition-all    ${isCollapsed ? 'justify-content-center' : 'gap-3'}   ${location.pathname === item.path ? 'bg-primary text-white shadow-sm' : 'text-dark bg-light-hover border border-transparent'}`} >
                                <FontAwesomeIcon icon={item.icon} style={{ width: '18px', minWidth: '18px' }} />
                                {!isCollapsed && <span className="fw-semibold small text-nowrap animate-fade-in">{item.label}</span>}
                            </Link>
                        </li>
                    ))}
                ...
                <div className="border-top pt-3 mt-auto d-flex align-items-center">
                    ...
                    {!isCollapsed && (
                        <div className="ms-2 overflow-hidden d-flex flex-column animate-fade-in">
